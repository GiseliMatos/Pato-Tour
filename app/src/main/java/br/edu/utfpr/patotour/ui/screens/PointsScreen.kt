         ...
         LazyColumn(
             ...
             item {
                 Row(
                     ...
                     Text(stringResource(R.string.registered_points), style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold)
                     Text(stringResource(R.string.points_count, filtered.size), color = MaterialTheme.colorScheme.onSurfaceVariant)
                 }
             }
             if (filtered.isEmpty()) {
 ...
 private fun EmptyState(onAdd: () -> Unit) {
     Column(Modifier.fillMaxWidth().padding(vertical = 48.dp), horizontalAlignment = Alignment.CenterHorizontally) {
         ...
         Text(stringResource(R.string.empty_points_hint), color = MaterialTheme.colorScheme.onSurfaceVariant, modifier = Modifier.padding(12.dp))
